...
data_directory = os.path.dirname(os.path.realpath(__file__)) + os.sep + 'data' + os.sep + 'chips'
cwd = os.getcwd()
#for python2:
try:
    FileNotFoundError
...
design_parameters['membrane_chip_02'] = dict(
                 ...
                 l_cap = 3.33e-3, #capillary length / [m]
                 p = 1e5, #chip pressure / 
                 T = 298,        
                                             )
...
class Chip:
    def __init__(self, name='unknown', chip_type='membrane_chip_02', 
                 updatefile=False, **kwargs):
        '''
        ...
        if not self.reset(): #reset returns false if there is no existing data file 
            print('Writing new file for Chip ' + name)
            file_name = name + '.txt'
            cwd = os.getcwd()
            ...
            os.chdir(cwd)
            for key, value in design_parameters[chip_type].items():
    ...
    def capillary_flow(self, gas='He',
                       w_cap=None, h_cap=None, l_cap=None, T=None, p=None
                           ):
        '''
