...
class Electrolyte:
    def __init__(self, name=None, electrolyte_type=None, s=None, concentration=None, pH=None,
                 cation=None, anion=None, spectator=None, E=None, F=None, p=None, verbose=True):
        '''
    ...
    def equilibrate(self, **kwargs):
        ...
        while len(done) < len(variables):  
            try:
                (names, fun) = [(n, f) for (n, f) in fundict.items()
                                if n[0] in done and n[1] in done
                                and n[2] not in done][0] 
            except IndexError:
...
#                str([getattr(self, names[0]), getattr(self, names[1])]) + ', a = ' + str(fun()))
   ...
   #         print(ion)
    ...
    def buffer_vec(self, pH):
        ...
        '''
        return np.array([np.product(self.Ka[0:i])/np.power(10.0, -pH*i) 
...
def electrolysis_ode(quantities, t, pars):
    ...
    quantitiesdict = {equilibrium_type[0]:quantities[0], equilibrium_type[1]:quantities[1]}
...
if __name__ == '__main__':
    ...
    el2 = Electrolyte(cation='K', anion='PO4---', concentration=1.0, pH=12, verbose=False
                      )      
    titrateit = False
    if titrateit:
        fig1 = plt.figure()
