...
class SettingsFile:
    ...
    def __init__(self, name, file_name=None, RE_vs_RHE=None, settings={}, verbose=True, **kwargs):
        ...
        try:
            ...
            print('why on earth don\'t you idiots just use utf-8 for everything? Trying iso8859_15.' )
    ...
    def change_potentials(self, RE_vs_RHE=None, diff=None, verbose=None):
        ...
        for n in range(N_lines): #len(self.lines changes each step when iterating over inumerate. Wierd.)
            line = self.lines[n]
            if line[0:9] == 'Technique':
                ...
                elif n<N_lines and len(self.lines[n+1])>5 and self.lines[n+1][0:6] in U_lines: 
                    N_step = 1
                    if verbose:
                        print('1 step')
                    newtech = False
                ...
                parts = [line[20*i:20*(i+1)] for i in range(int(N_step+1))] #+1 for line title
#                print(parts)
                if len(parts) == 0 or n == N_lines:
                    ...
                    if parts[0][0:6] in U_lines:
                        ...
                        check_parts = [check_line[20*i:20*(i+1)] for i in range(int(N_step+1))] #+1 for line title
                        newline = ''
                        for part, check_part in zip(parts, check_parts):
                            ...
                            if check_part[0:3] == 'Ref': 
                                ...
                                if verbose:
                                    print('replacing ' + str(U) + ' V vs Ref with ' + str(U_new) + ' V vs Ref.')
...
if __name__ == '__main__':
    data_dir = os.path.expanduser('~/Dropbox/Sniffer_Experiments/06_HER/Data/17G10_Nik3/')
    os.chdir(data_dir)
    test = SettingsFile(name='test', file_name = 'closing_secondary.mps', RE_vs_RHE=1.298, verbose=True)
