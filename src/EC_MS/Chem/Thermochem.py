...
standard_redox = {'H':+1, 'O':-2, 'N':-3, 'F':-1, 'Cl':-1, 'Br':-1, 'I':-1,
                  'S':-2, 'Li':+1, 'Na':+1, 'K':+1, 'Rb':+1, 'Cs':+1,
                  'e':-1, #electrons come with a negative charge
                  '-':+1, '+':-1 # If charges are counted this way, then
...
kH0 = { #Henry's Law constant of volatility in bar*l/mol
       'N2': 1660.87, 'CO2': 29.87, 'H2O': 0.0005791, 'CH3CH2OH': 0.0047595,
...
def get_standard_state(comp, states={}, verbose=True, out='cs'):
    ...
    if s is not None:
        ...
        try:
            ...
            if verbose:
                print('no state given for ' + comp + '.\n' +
                      'Input it as, e.g. states={\''+ comp + '\':\'aq\'}')
...
#        print('c = ' + c + ', n = ' + str(n)) #for debugging
        try:
            ...
            try:
                ...
                if verbose:
                    print('no standard entropy available for ' + c +
                          '. Couldn\'t get dfS for ' + comp + '. Returning None.')
                return None
    return dfS0
...
def get_dfH(comp, T=None, verbose=True):
    ...
    try:
        ...
        try:
            ...
            if verbose:
                print('no standard enthalpy available for ' + comp +
                      '. Returning None.')
            return None
    return dfH
...
def get_drG(nu, states={}, dfG={}, verbose=True):
    ...
    for comp, n in nu.items():
        ...
        if dfGc is None:
            print('no free energy of formation available for ' + comp + '.\n' +
                  'Input it as, e.g. dfG={\'' + comp + '\':0}.\n'+
...
def get_kH(comp=None, T=None, dfG={}, kH_0=None, dsH=None, T0=298.15,
           verbose=True, tryG=True):
    '''
...
def get_oxidation_state(comp, atom='C', redox_states={}):
    ...
    for element, n in elements.items():
        ...
        try:
            ...
            print('I don\'t know the oxidation state of ' + element + '\n' +
                  'Input it in as redox_states = {atom: oxidation_state}.\n' +
                  'For now, I\'ll assume it\'s zero.')

    return - ro / n_comp

def get_rxn_EC(product='CH4', reactant='CO2', atom='C', redox_states={},
    ...
    if not set(get_elements(nu).keys()).issubset({atom, 'O','H','+','-','e'}):
        print('function Chem.get_rxn_EC: WARNING!\n'+
...
def get_rxn_cell(rxn_an, rxn_cat, out='string'):
    ...
    n = - n_an * n_cat / gcd(n_an, n_cat)  # positive if both are forward or both backwards
    nu = {}
